...
std::string getRandomString(const int strSize)
{
	...
	engine.seed(device());
	std::uniform_int_distribution<int> char_distribution(97, 122); // ASCII codes from a to z
...
std::string& changeSomeChars(std::string& str, size_t numChars) // change ~numChars of str chars
{
	size_t size = str.size();
	if (numChars > size) { numChars = size; }
	std::mt19937 engine;
	std::random_device device;
	engine.seed(device());
	std::uniform_int_distribution<int> char_distribution(97, 122); // ASCII codes from a to z
	std::uniform_int_distribution<int> char_position(0, size-1);   // index of str
...
Result getParallelCompare(const std::string& a, const std::string& b)
{
	...
	if (procRank == 0)
	{
		...
		size_t additionLen = (procNum - remainder) % procNum;  // addition to the original strings to divide them into equal parts
	...
	MPI_Scatter(a_copy, delta, MPI_CHAR,
				a_part, delta, MPI_CHAR,
				0, MPI_COMM_WORLD);
	MPI_Scatter(b_copy, delta, MPI_CHAR,
				b_part, delta, MPI_CHAR,
				0, MPI_COMM_WORLD);
...
next:
	...
	if (procRank == 0)
	{
		...
		MPI_Gather(&int_result, 1, MPI_INT,
			allResults.data(), 1, MPI_INT,
			0, MPI_COMM_WORLD);
