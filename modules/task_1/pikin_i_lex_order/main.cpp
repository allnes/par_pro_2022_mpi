...
TEST(Parallel_Comparison, Small_test) {
    ...
    if (rank == 0) {
        ...
        s2 = s1;
        changeSomeChars(s2, 3);

        std::cout << "A: "<< s1 << std::endl << "B: " << s2 << std::endl;
        sequentialResult = getSequentialCompare(s1, s2);
        std::cout <<"Sequential result: A is " << getStrFromRes(sequentialResult) << " B" << std::endl;
    }

    Result parallelResult = getParallelCompare(s1,s2);

    if (rank == 0) {
        std::cout << "Parallel result: A is " << getStrFromRes(parallelResult)<< " B" << std::endl;
...
TEST(Parallel_Comparison, Compare_strings_with_diff_len) {
    ...
    if (rank == 0) {
        ...
        s1 = std::string(s2, 0ll, 100000ll); // s1 - prefix of s2 => s1<s2
