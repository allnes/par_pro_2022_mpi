...
int CountingAlphabeticCharParallel(const std::string& str) {
  ...
  if (rank == 0) {
    for (int proc = 1; proc < size; proc++) {
      MPI_Send(str.c_str() + (proc * delta), delta, MPI_CHAR, proc, 0,
               MPI_COMM_WORLD);
    }
  }
  ...
  if (rank == 0) {
    local_string = std::string(str.begin(), str.begin() + delta);
  } else {
    MPI_Status status;
    MPI_Probe(0, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
    MPI_Get_count(&status, MPI_CHAR, &delta);
    local_string.resize(delta);
    MPI_Recv(&(local_string[0]), delta, MPI_CHAR, 0, 0, MPI_COMM_WORLD,
             &status);
  }
